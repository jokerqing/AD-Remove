  ...
  $nobyda.post(bilibili, function(error, response, data) {
    if (!error) {
      if (response.status == 200) {
        ...
        $nobyda.notify("BOC - 签到成功！🎉", "", "")
      } else {
        console.log("bilibili failed response : \n" + data)
        if (data.match(/duplicate/)) {
          $nobyda.notify("BOC - 今日已签过 ⚠️", "", "")
        } else if (data.match(/uid must/)) {
          $nobyda.notify("BOC - Cookie无效 ‼️‼️", "", "")
        } else {
          $nobyda.notify("BOC - 签到失败 ‼️", "", data)
        }
      }
    } else {
      $nobyda.notify("BOC - 签到接口请求失败", "", error)
    }
  })
}
